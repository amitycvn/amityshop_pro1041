...
public class ChiTietSanPhamRepository implements MethodRepository<ChiTietSanPham> {
    ...
    String GET_ALL = """
                     select * from chi_tiet_san_pham where deleted = 0 order by ID OFFSET ? ROWS FETCH NEXT ? ROWS ONLY
                     """;
    String GET_ALL_DATA = """
                     select * from chi_tiet_san_pham where deleted = 0
                     """;
    String GET_ALL_DATA_REMOVE = """
                     select * from chi_tiet_san_pham where deleted = 1
                     """;
    String GET_BY_ID = """
                       select * from chi_tiet_san_pham where id = ?
                       """;
    String GET_BY_PROPERTIES = """
                            select * from chi_tiet_san_pham where id like ? and deleted = 0
                            """;
    String INSERT = """
    ...
    String GET_BY_NAME = """
                    ...
                    san_pham sp on ctsp.id_san_pham = sp.id where sp.ten like ?
                    """;
    String GET_CTSP_BY_ID_SP = """
                               select * from chi_tiet_san_pham where id_san_pham like ?
                               """;
    public List<ChiTietSanPham> getCTSPByIDSP(String id){
        String value = "%"+id+"%";
        return selectBySQL(GET_CTSP_BY_ID_SP, value);
    }
    @Override
    ...
    public void add(ChiTietSanPham o) {
        JDBCHelper.executeUpdate(INSERT, 
                ...
                o.getGiaBan(), o.getMoTa(), o.getTrangThai());
    }
    ...
    public List<ChiTietSanPham> getByTen(String value){
        String name = "%"+ value + "%";
        return selectBySQL(GET_BY_NAME, name);
    }
    
    // Hy hữu lắm mới có mã giống nhau
    public List<ChiTietSanPham> getByMaCTSP(String value){
        String name = "%"+value+"%";
        return selectBySQL(GET_BY_PROPERTIES, name);
    }
...
//        }
        for (ChiTietSanPham o : repository.getByTen("Áo phông s")) {
            System.out.println(o);
        }
    }
