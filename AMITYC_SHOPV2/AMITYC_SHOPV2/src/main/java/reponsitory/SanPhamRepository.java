...
public class SanPhamRepository implements MethodRepository<SanPham> {
    ...
    String GET_ALL = """
                    select * from san_pham where deleted = 0  order by ID OFFSET ? ROWS FETCH NEXT ? ROWS ONLY
                    """;
    String GET_ALL_DATA = """
                    select * from san_pham where deleted = 0
                    """;
    String GET_ALL_DATA_REMOVE = """
                             select * from san_pham where deleted = 1
                             """;
    ...
    String GET_BY_PROPERTIES = """
                        select * from san_pham where (ten like ? or ma_san_pham like ? or trang_thai like ?) and deleted = 0
                        """;

    String GET_BY_PROPERTIES_UNREMOVE = """
                        select * from san_pham where (ten like ? or ma_san_pham like ? or trang_thai like ?) and deleted = 1
                        """;
    String INSERT = """
    ...
    public List<SanPham> getByProperties(String value) {
        String properties = "%" + value + "%";
        return selectBySQL(GET_BY_PROPERTIES, properties, properties, properties);
    }
